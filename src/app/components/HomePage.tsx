...
export function HomePage({ onNavigate }: HomePageProps) {
    ...
    const usedProducts = useMemo(() =>
        products.filter(p => p.condition === 'used').slice(0, 6),
        [products]
    );
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <section className="max-w-[1400px] mx-auto px-3 sm:px-4 lg:px-6 py-10 sm:py-14 bg-gray-50/50 rounded-3xl mt-4">
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 sm:mb-8">
                    ...
                    <button onClick={() => onNavigate('products')} className="text-sm font-medium text-gray-700 hover:text-black flex items-center gap-2 transition-colors">
            ...
            <section className="max-w-[1400px] mx-auto px-3 sm:px-4 lg:px-6 py-10 sm:py-14">
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 sm:mb-8">
                    <div>
                        ...
                        <h2 className="mt-1 text-2xl sm:text-3xl font-semibold tracking-tight text-gray-900">Browse products</h2>
                        ...
                        <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
                            <SheetTrigger asChild>
                                <Button variant="outline" className="h-9 text-sm gap-1.5">
                                    <SlidersHorizontal className="h-4 w-4" /> Filters {hasActiveFilters && '!'}
                                </Button>
                            </SheetTrigger>
                            <SheetContent side="left" className="w-[300px] overflow-y-auto pl-7 pr-6">
                                ...
                                <div className="mt-5 space-y-6">
                                    <div>
                                        ...
                                        <div className="space-y-2">{categories.map(c => (
                                            <label key={c.id} className="flex items-center gap-2 cursor-pointer text-xs font-medium">
                                                <Checkbox checked={selectedCategories.includes(c.id)} onCheckedChange={(checked) => onCategoryChange(c.id, !!checked)} />
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-2">{brands.map(b => (
                                            <label key={b} className="flex items-center gap-2 cursor-pointer text-xs font-medium">
                                                <Checkbox checked={selectedBrands.includes(b)} onCheckedChange={(checked) => onBrandChange(b, !!checked)} />
                                                {b}
                                            </label>
                                        ))}</div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <Slider min={0} max={PRICE_MAX} step={10} value={priceRange} onValueChange={(v) => setPriceRange(v as [number, number])} className="w-full" />
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                    {filteredProducts.slice(0, 10).map((product, index) => (
                        <motion.div key={product.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }}>
                            <ProductCard product={product} onNavigate={onNavigate} />
                        </motion.div>
                    ))}
                </div>
            ...
            <section className="max-w-[1400px] mx-auto px-3 sm:px-4 lg:px-6 py-10 sm:py-14 bg-gray-50/20">
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 sm:mb-8">
                    ...
                    <button onClick={() => onNavigate('used')} className="text-sm font-medium text-black-700  flex items-center gap-1.5 bg-gray-100 px-4 py-2 rounded-full transition-colors">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                    {usedProducts.length > 0 ? (
                        usedProducts.slice(0, 5).map((product, index) => (
                            <motion.div key={product.id + '-used'} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
                                <ProductCard product={product} onNavigate={onNavigate} />
                            </motion.div>
                        ))
                    ) : (
            ...
            <section className="max-w-[1400px] mx-auto px-3 sm:px-4 lg:px-6 py-10 sm:py-14">
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 sm:mb-8">
                    <div>
                        ...
                        <h2 className="mt-1 text-2xl sm:text-3xl font-semibold tracking-tight text-gray-900">Shop by Category</h2>
                    </div>
                    <button onClick={() => onNavigate('categories')} className="text-sm font-medium text-gray-600 hover:text-gray-900 flex items-center gap-1.5 transition-colors">
                        View all <ArrowRight className="h-4 w-4" />
                    </button>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
                    {categories.slice(0, 6).map((category, index) => (
                        <motion.button key={category.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} onClick={() => onNavigate('products', category.id)} className="group text-left">
                            ...
                            <h3 className="font-semibold text-sm group-hover:text-gray-600 transition-colors uppercase tracking-tight">{category.name}</h3>
                        </motion.button>
                    ))}
                </div>
            </section>
