...
export function ProductCard({ product, onNavigate }: ProductCardProps) {
  ...
  const originalNum = typeof product.originalPrice === 'number' ? product.originalPrice : undefined;

  const discount =
    typeof originalNum === 'number' && originalNum > 0 && priceNum > 0
    ...
    >
      ...
      <div className="space-y-0.5 sm:space-y-1">
        ...
        <div className="flex items-center gap-1.5">
          <span className="font-semibold text-sm">Birr {priceNum.toFixed(2)}</span>

          {typeof originalNum === 'number' && originalNum > priceNum && (
            <span className="text-xs text-gray-400 line-through">Birr {originalNum.toFixed(2)}</span>
          )}
        </div>
      </div>
    </motion.div>
